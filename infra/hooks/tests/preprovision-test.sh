...
run_hook() {
  ...
  env \
    ...
    ACCEPT_BING_GROUNDING_TERMS='true' \
...
run_hook "${migration_log}" \
  FOUNDRY_PROJECT_ENDPOINT='https://portable-foundry.services.ai.azure.com/api/projects/test' \
  CONTENT_SAFETY_ENDPOINT='https://portable-safety.cognitiveservices.azure.com/'

if ! grep -q 'azd env set AZURE_FOUNDRY_ACCOUNT_NAME portable-foundry' "${migration_log}"; then
  echo 'Existing Foundry account name was not preserved from its endpoint.' >&2
  exit 1
fi
if ! grep -q 'azd env set AZURE_CONTENT_SAFETY_ACCOUNT_NAME portable-safety' "${migration_log}"; then
  echo 'Existing Content Safety account name was not preserved from its endpoint.' >&2
  exit 1
fi
...
run_hook "${valid_override_log}" \
  ...
  AZURE_FOUNDRY_ACCOUNT_NAME='portable-foundry-123' \
